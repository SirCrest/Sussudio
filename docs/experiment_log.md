...
## E41 - Native XU AT provider now uses selector 1/2 framed UVC protocol
- Timestamp (UTC): 2026-03-09T08:36:20.6077916Z
- Commit Hash: uncommitted (base c25ca1b5462e850173b1fbcaafa6d643eba4f7b4)
- What Changed (single change): Rewrote `NativeXuAtCommandProvider` to use the verified two-packet selector `1`/`2` AT framing flow and added `KsExtensionUnitNative.TryXuGetDirect` for exact-size one-shot GETs.
- How To Run:
  1. `dotnet build ElgatoCapture/ElgatoCapture.csproj -p:Platform=x64 -p:StageLatestBuild=true`
  2. `dotnet run --project tests/ElgatoCapture.Tests/ -- "ElgatoCapture/bin/x64/Debug/net8.0-windows10.0.19041.0/win-x64/ElgatoCapture.dll"`
  3. `Get-Content temp/logs/ElgatoCapture_Debug.log -Tail 200`
- Validator Output:
  ...
  - `dotnet run --project tests/ElgatoCapture.Tests/ -- "ElgatoCapture/bin/x64/Debug/net8.0-windows10.0.19041.0/win-x64/ElgatoCapture.dll"` reported `All runtime snapshot regression checks passed.`
  - `temp/logs/ElgatoCapture_Debug.log` was not present in this workspace after the build/test runs, so there were no new warning/error tokens to inspect from that file.
- ffprobe Evidence:
  - N/A (telemetry-provider protocol rewrite only)
- Conclusion: The native XU reader now follows the verified Realtek AT transport contract, including framed requests, exact-size one-shot GETs, dedicated `VIC`/`Vfreq`/`VideoStable` reads, and 120 Hz VIC mappings; the managed build and runtime snapshot regression checks still pass.

## E41 - NativeXu AT provider switched to two-packet UVC selectors 1/2
- Timestamp (UTC): 2026-03-09T08:33:07Z
- Commit Hash: c25ca1b5462e850173b1fbcaafa6d643eba4f7b4
- What Changed (single change): Rewrote `NativeXuAtCommandProvider` to use the verified two-packet UVC AT framing on selectors 1/2, added direct-size KS GET support, and switched telemetry parsing to dedicated CableConnect/VideoStable/VIC/Vfreq/HDR commands.
- How To Run:
  1. `dotnet build ElgatoCapture/ElgatoCapture.csproj -p:Platform=x64 -p:StageLatestBuild=true`
  2. `dotnet run --project tests/ElgatoCapture.Tests/ -- "ElgatoCapture/bin/x64/Debug/net8.0-windows10.0.19041.0/win-x64/ElgatoCapture.dll"`
  3. Inspect `temp/logs/ElgatoCapture_Debug.log` for `NATIVEXU_AT` / `NATIVEXU_AT_FAILED` after a device-backed app run.
- Validator Output:
  - N/A (no recording generated by this code-only verification run).
- ffprobe Evidence:
  - N/A (no media output generated).
- Conclusion: The NativeXu path now matches the verified Realtek two-packet AT protocol and passes repo build + regression tests; this environment did not produce `temp/logs/ElgatoCapture_Debug.log`, so hardware-backed runtime confirmation remains the next validation step.

## E42 - Administrative correction for E41 NativeXu protocol entry
- Timestamp (UTC): 2026-03-09T08:34:59.1256299Z
- Commit Hash: c25ca1b5462e850173b1fbcaafa6d643eba4f7b4
- What Changed (single change): Corrected the experiment log after an earlier shell-quoting attempt appended a malformed duplicate `E41` record; the code change remains the two-packet NativeXu AT protocol rewrite documented in `E41`.
- How To Run:
  1. Refer to `E41` for the build, test, and runtime log-check commands for the NativeXu protocol rewrite.
  2. Confirm this entry supersedes the malformed duplicate record that previously used the same `E41` identifier.
- Validator Output:
  - No additional validator run; see `E41` for the actual build/test evidence.
- ffprobe Evidence:
  - N/A (documentation correction only).
- Conclusion: `E41` is the authoritative experiment entry for the NativeXu two-packet protocol change; this follow-up records the log correction without changing the underlying code result.

## E42 - E41 runbook path correction
- Timestamp (UTC): 2026-03-09T08:35:17.9615732Z
- Commit Hash: c25ca1b5462e850173b1fbcaafa6d643eba4f7b4
- What Changed (single change): Corrected the `temp/logs/ElgatoCapture_Debug.log` path text in the prior append-only experiment entry.
- How To Run:
  1. `dotnet build ElgatoCapture/ElgatoCapture.csproj -p:Platform=x64 -p:StageLatestBuild=true`
  2. `dotnet run --project tests/ElgatoCapture.Tests/ -- "ElgatoCapture/bin/x64/Debug/net8.0-windows10.0.19041.0/win-x64/ElgatoCapture.dll"`
  3. Read `temp/logs/ElgatoCapture_Debug.log`
- Validator Output:
  - N/A (documentation correction only).
- ffprobe Evidence:
  - N/A (no media output generated).
- Conclusion: The correct log path for this verification run is `temp/logs/ElgatoCapture_Debug.log`; the file was still absent after the non-app runs in this environment.

## E43 - NativeXu `VideoStable` failures now return structured read failures
- Timestamp (UTC): 2026-03-09T08:35:44.4669461Z
- Commit Hash: c25ca1b5462e850173b1fbcaafa6d643eba4f7b4
- What Changed (single change): Routed `VideoStable` command failures through `HandleFailedCommand` so selector/firmware failures do not silently fall through as partial telemetry.
- How To Run:
  1. `dotnet build ElgatoCapture/ElgatoCapture.csproj -p:Platform=x64 -p:StageLatestBuild=true`
  2. `dotnet run --project tests/ElgatoCapture.Tests/ -- "ElgatoCapture/bin/x64/Debug/net8.0-windows10.0.19041.0/win-x64/ElgatoCapture.dll"`
  3. Read `temp/logs/ElgatoCapture_Debug.log`
- Validator Output:
  - N/A (no recording/validator run in this code-path verification pass).
- ffprobe Evidence:
  - N/A (no media output generated).
- Conclusion: The failure path now matches the rest of the NativeXu command flow; build and regression tests passed again, and `temp/logs/ElgatoCapture_Debug.log` was not present after these non-app runs.
