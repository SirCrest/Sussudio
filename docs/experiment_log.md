...
## 2026-05-06 — LibAv recording fair-drain checkpoint

**Scope.** First surgical restructuring checkpoint for development safety and runtime reliability. The target was normal recording queue fairness, specifically the risk that `LibAvRecordingSink` could drain an unbounded CUDA/GPU/video backlog before servicing HDMI or microphone audio.

**Change.** `LibAvRecordingSink` now mirrors the Flashback sink's safer drain shape: audio and microphone queues are drained first, CUDA/GPU/raw video queues are drained in bounded batches, then audio and microphone are drained again to catch samples that arrived during video encoding. The libav single-thread ownership model is unchanged; only queue service order and batch limits changed. Added `LibAvRecordingSink_NormalDrainLoopInterleavesAudioWithBoundedVideoBatches` to pin the constants, bounded drain overloads, and audio-before/after-video ordering.

**Verification.** Baseline `dotnet build Sussudio.slnx --no-restore` passed before edits. After the change, `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including the new fair-drain guard, and `dotnet build Sussudio.slnx --no-restore` passed. This is static and build-level proof only; live recording/Flashback diagnostic sessions are still required before claiming a measured audio-continuity or frame-pacing improvement.

## 2026-05-06 — Non-blocking video enqueue checkpoint

**Scope.** Second runtime-safety checkpoint for the restructuring goal. The target was source-reader fanout: recording and Flashback `TryEnqueue*VideoFrame` paths were named like non-blocking operations but still retried full video queues for up to 250ms, which could keep the capture callback from reaching preview submission.

**Change.** `LibAvRecordingSink` raw/GPU/CUDA video admission now performs one immediate queue write attempt. Queue saturation releases the packet resources immediately, fails normal recording loudly through the existing fatal path, and returns `Overloaded` to the caller for the existing overload counters/logs. `FlashbackEncoderSink` raw/GPU video admission now also performs one immediate queue write attempt; queue saturation releases packet resources, records `queue_full` rejection telemetry, and returns `Overloaded` without making Flashback fatal. The old retry sleeps and 250ms backpressure timeout constants were removed from video enqueue paths. Source-level regression guards now pin that these paths do not spin, sleep, or wait on encoder queue backpressure.

**Verification.** `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including `Recording video try enqueue paths do not block capture callbacks`, the updated Flashback queue-depth guard, and the prior fair-drain guard. `dotnet build Sussudio.slnx --no-restore` passed with 0 warnings and 0 errors. This remains source/build validation; a live diagnostic session should still verify preview pacing, normal recording, Flashback playback/export, and audio continuity under real device load before claiming measured runtime improvement.

## 2026-05-06 — Hot audio write contract checkpoint

**Scope.** Third runtime-safety checkpoint for the restructuring goal. The target was the WASAPI callback write contract: HDMI and microphone capture invoke recording/Flashback audio writers on the capture thread, so any future async or blocking implementation would create a direct audio glitch risk.

**Change.** `IRecordingSink.WriteAudioAsync` now documents the hot-path contract: implementations must copy/enqueue synchronously, avoid blocking/async work, and return a completed task. `WasapiAudioCapture` now routes delegate, recording, and Flashback writes through a shared completion guard that rejects incomplete tasks instead of waiting on them in the callback path. `LibAvRecordingSink` and `FlashbackEncoderSink` audio/microphone write methods now carry explicit hot-path comments and continue to return `Task.CompletedTask` after bounded queue admission.

**Verification.** `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including `WASAPI audio capture rejects incomplete hot audio writes`, the non-blocking video enqueue guard, and the LibAv fair-drain guard. `dotnet build Sussudio.slnx --no-restore` passed with 0 warnings and 0 errors. Live audio-continuity validation is still required before claiming runtime improvement, but the source contract now fails loudly if someone accidentally returns an incomplete audio write task on the WASAPI callback thread.

## 2026-05-06 — Flashback backend ownership aggregate checkpoint

**Scope.** First ownership/structure checkpoint for the restructuring goal. The target was `CaptureService` retaining separate authoritative fields for Flashback sink, buffer manager, exporter, playback controller, and backend settings snapshot, which made teardown/cycle paths easy to drift.

**Change.** Added `FlashbackBackendResources` as the authoritative ownership aggregate for the preview-owned Flashback backend. `CaptureService` now routes the existing private Flashback component accessors through this aggregate, uses aggregate helpers for resource presence, install, playback-controller take/clear, sink/settings clear, full clear, and repeated producer detach during backend dispose/cycle. This keeps `CaptureService` as the transition coordinator while moving backend resource ownership and producer-detach behavior into a focused Flashback collaborator. Existing public/runtime contracts are preserved.

**Verification.** `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed after updating stale source guards to the aggregate ownership API, including `CaptureService flashback backend ownership uses resource aggregate`, `Retained flashback preview pipeline recycles on settings changes`, and `Flashback lifecycle logs use outcome names`. `dotnet build Sussudio.slnx --no-restore` then passed with 0 warnings and 0 errors. This is a structure/build checkpoint only; it does not yet complete the broader Flashback ownership extraction for export/finalize/preserve behavior.

## 2026-05-06 — Automation command catalog checkpoint

**Scope.** Tooling-safety checkpoint for the restructuring goal. The target was automation contract drift across the app dispatcher, shared pipe protocol, `ssctl`, MCP, diagnostic sessions, and source-guard tests.

**Change.** Added a shared `AutomationCommandCatalog` in `tools\Common` with command metadata for payload shape, readiness gating, response timeout, path policy, CLI help text, and MCP description text. `AutomationPipeProtocol.GetDefaultResponseTimeout()` now delegates timeout policy to the catalog, and `AutomationCommandDispatcher.RequiresReadyDevices()` delegates readiness policy to the same metadata while preserving the existing switch-based command execution. Server-side path-bearing automation requests now pass through catalog-backed validation/parent-directory creation for `SetOutputPath`, `FlashbackExport`, `VerifyFile`, `CapturePreviewFrame`, and `CaptureWindowScreenshot`. Long-running `FlashbackExport` now uses the Flashback mutation timeout policy (`305000ms`) instead of the shorter extended timeout (`60000ms`) across shared protocol defaults, MCP, and diagnostic-session concurrent/disable-during-export paths. The pass also fixed `ssctl device audio-select` to send the dispatcher-compatible `deviceName` payload key.

**Verification.** Built the directly affected tool binaries with `dotnet build tools\ssctl\ssctl.csproj --no-restore`, `dotnet build tools\McpServer\McpServer.csproj --no-restore`, and `dotnet build tools\AutomationClient\AutomationClient.csproj --no-restore`. Built the app path used by the regression harness with `dotnet build Sussudio.slnx --no-restore`. The first full test run exposed stale x64 app/probe outputs; after rebuilding the solution and forcing `dotnet build tools\NativeXuAudioProbe\NativeXuAudioProbe.csproj -c Debug --no-restore -t:Rebuild`, `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including the new `Automation command catalog covers command metadata and policy` guard and the updated Flashback export timeout checks. This is contract/build validation only; live automation should still run Flashback export diagnostics before claiming runtime export reliability improvement.

## 2026-05-06 — Capture mode options builder checkpoint

**Scope.** UI/ViewModel simplification checkpoint for the restructuring goal. The target was the pure option-building logic inside `MainViewModel.DeviceManagement`, specifically resolution-option construction/filtering and selected-mode video-format option construction.

**Change.** Added `CaptureModeOptionsBuilder` as a pure helper for building resolution options from the resolution-to-format matrix and source telemetry, including HDR enablement flags and source aspect-ratio filtering. The same builder now creates video-format option lists by normalizing, deduplicating, and sorting pixel formats while preserving the `Auto` option. `MainViewModel.DeviceManagement` still owns selection state, source retargeting, and reinitialization decisions, but no longer carries these deterministic list-building helpers inline.

**Verification.** `dotnet build Sussudio.slnx --no-restore` passed. `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including the new `Capture mode options builder builds resolution and video format options` regression guard. This is a focused structure/test checkpoint; it does not yet extract the larger frame-rate selection state machine or stats presentation logic.

## 2026-05-06 — Restructuring live diagnostic smoke

**Scope.** Live automation smoke for the surgical restructuring checkpoints: non-blocking recording/Flashback video fanout, fair LibAv recording drain policy, explicit hot audio write contracts, first Flashback backend ownership aggregate, shared automation command metadata/timeouts/path policy, and the capture mode options builder.

**Verification.** Launched the freshly built Debug app from `Sussudio\bin\Debug\net8.0-windows10.0.19041.0\win-x64\Sussudio.exe` and confirmed `ssctl state --json` reported automation initialized, previewing, Flashback active, audio signal present, and a healthy 3840x2160 MJPG 120fps live path. Short live diagnostic sessions then passed:

- `temp\diagnostic-sessions\20260507_022147` (`observe`, 10s, verify): `Success=true`, `HealthStatus=Healthy`, visual output/change cadence about `120fps`, repeat about `0.097%`, no warnings.
- `temp\diagnostic-sessions\20260507_022210` (`recording-only`, 10s, verify): `Success=true`, strict recording verification passed, Flashback recording backend and file growth observed, `FlashbackRecordingVideoFramesSubmittedDelta=1202`, packets written `1202`, sequence-gap delta `0`, queue-drop delta `0`, no warnings.
- `temp\diagnostic-sessions\20260507_022313` (`flashback-playback`, 10s, verify): `Success=true`, playback returned Live, `FlashbackPlaybackFrameCountAtEnd=1230`, observed playback `119.98fps`, 1% low `119.93fps`, dropped-frame delta `0`, submit-failure delta `0`, no warnings.
- `temp\diagnostic-sessions\20260507_022333` (`flashback-range-export`, 10s, verify): `Success=true`, range export verified, `FlashbackExportStatusAtEnd=Succeeded`, output `flashback-range-export.mp4`, message `Exported 834 packets from 1 segments`, max export elapsed `7630ms`, strict verification passed, no warnings.

**Caveat.** These are deliberately short smoke diagnostics, not the long 180s-300s soak runs used for go-live claims. They do prove the edited build can preview, record, play Flashback, export Flashback, and pass strict artifact verification through the shared automation/tooling path without immediate regression.

## 2026-05-06 — Stats presentation builder checkpoint

**Scope.** UI/ViewModel simplification checkpoint for the restructuring goal. The target was `MainWindow.StatsOverlay.cs`, which mixed WinUI control updates with pure stats text formatting, metric status resolution, diagnostic row parsing, frame-time range math, and live diagnostic summary classification.

**Change.** Added `StatsPresentationBuilder` as the focused presenter/model layer for stats surfaces. The builder now owns dock text/status projection, frame-time overlay text and range data, telemetry/diagnostic row presentation, and the stats diagnostic health summary used by `GetStatsSnapshot()`. `MainWindow.StatsOverlay.cs` now keeps the UI-specific responsibilities: polling, visibility, row pools, metric brushes, and canvas line drawing.

**Verification.** `dotnet build Sussudio.slnx --no-restore` passed after the extraction. `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including the new `Stats presentation logic lives in focused builder` guard plus the updated stats source-telemetry, live-summary, and frame-time overlay guards. This is a structure/test checkpoint; no live diagnostic rerun was needed because the edit is presentation-only and does not touch capture, recording, Flashback, HDR, or automation dispatch behavior.

## 2026-05-06 — Flashback preserve policy ownership checkpoint

**Scope.** Follow-up ownership/structure checkpoint for the restructuring goal. The target was the Flashback recovery-preserve and segment-purge policy, which still lived as a standalone `CaptureService` field even though it controls whether backend segment files are allowed to be purged during dispose/cleanup after failed recording finalize paths.

**Change.** Moved the recovery-preserve flag and preserve/purge decisions into `FlashbackBackendResources`. The aggregate now exposes `ClearRecoveryPreserve()`, `ResolveSegmentPurge(...)`, and `PreserveRecoverySegments(...)`, and it marks the active `FlashbackBufferManager` session for recovery when finalize/cycle cleanup needs to preserve artifacts. `CaptureService` remains the transition coordinator but no longer owns that backend-preservation state directly.

**Verification.** `dotnet build Sussudio.slnx --no-restore` passed after the ownership move. `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed, including the updated Flashback backend ownership guard and recovery-preserve source checks. This is a low-risk ownership extraction; it intentionally does not alter Flashback segment eviction/recovery internals or exporter timestamp remapping.

## 2026-05-07 — Flashback recording finalize ownership checkpoint

**Scope.** Follow-up ownership/structure checkpoint for the restructuring goal. The target was Flashback recording finalize orchestration: ending the preview-owned recording backend, capturing final integrity counters before export, preserving tail artifacts on export failure, and resuming paused eviction.

**Change.** `FlashbackBackendResources` now owns `FinalizeRecordingAsync(...)` for the Flashback recording backend. The aggregate pauses eviction, calls `FlashbackEncoderSink.EndRecordingAsync(...)`, invokes the boundary snapshot callback before export, delegates timestamp-sensitive export work back to `CaptureService.ExportFlashbackCoreAsync(...)`, preserves end artifacts when the final export fails, logs recording-export success/failure, and resumes eviction through the existing best-effort helper. `CaptureService` remains the public transition coordinator and still owns exporter timestamp remapping, live-edge completeness, diagnostics, and recording-integrity presentation.

**Verification.** `dotnet run --project tests\Sussudio.Tests\Sussudio.Tests.csproj --no-restore` passed after updating the source guards to the new ownership boundary. `dotnet build Sussudio.slnx --no-restore` passed with 0 warnings and 0 errors. No additional live diagnostic session was run for this checkpoint because the edit moves orchestration ownership only and does not change encoder math, exporter timestamp remapping, source-reader fanout, HDR policy, or automation command behavior; the earlier restructuring live diagnostic smoke remains the live regression evidence for the current edited build.
