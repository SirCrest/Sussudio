...
`MainWindow.CaptureBindings.cs` left as the XAML-facing capture and
...
backend disposal, WASAPI disposal, and microphone cleanup;
...
shared unified-video cleanup mechanics still delegate to the video pipeline
resource owner;
...
`LibAvRecordingSink.VideoQueueSubmission.cs`. `LibAvRecordingSink.Startup.cs` owns the
...
plus the HDMI/Analog codec switch sequence used by `SwitchAudioInputAsync` and
the analog gain register mapping/writes used by `SetAnalogGainAsync`.
...
classification, force-rotate audio queue guard policy, producer wakeup
signaling, disposed/not-started/cancelled/force-rotate/failure rejection
reasons, video/GPU input validation, hot audio packet enqueue, channel writes,
queue-depth increments, max-depth updates, failed-write depth rollback,
...
state, mutation, reset, and read-only projections so the root renderer stays
focused on construction, constants, user-facing state accessors, and public
state changes.
...
reuse, shared stats dock row creation, text mutation, visibility toggles, row
style application, diagnostic row presentation, telemetry diagnostics empty
state, group headers, and diagnostic row pooling, while
...
clamp/positioning.
`Sussudio/MainWindow.Flashback.Interactions.cs` is the XAML-facing playhead
...
`Sussudio/Controllers/Flashback/FlashbackUiControllers.cs`.
`Sussudio/MainWindow.Flashback.Interactions.cs` is the XAML-facing polling
...
`Sussudio/MainWindow.CaptureBindings.cs` adapter. Output-path routing
...
router; `Sussudio/MainWindow.AudioBindings.cs` is the XAML-facing
...
`Sussudio/MainWindow.CaptureBindings.cs` keeps controller
...
`Sussudio/Controllers/Capture/CaptureOptionPresentationController.cs`.
`MainWindow.CaptureBindings.cs` is the XAML-facing adapter and keeps
...
`Sussudio/MainWindow.CaptureBindings.cs` now owns the XAML-facing
...
changes.
`MainWindow.CaptureBindings.cs` keeps the old capture and recording option
