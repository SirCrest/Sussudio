...
Keep decoder creation, active segment file identity, file open checks, decoder
close/open identity transitions, and decoder cleanup there. Active fMP4 reopen
retry, adjacent-segment seek fallback, keyframe-reopen recovery, and near-live
reopen guards now live in
...
The detailed command queue, cadence/1% low, decode timing,
audio-master/A/V drift, and stage/seek DTO value maps live in
`DiagnosticSessionResultBuilder.FlashbackPlaybackMetricResult.cs`, with the
...
4. Move MainViewModel feature state behind a facade.
   ...
   while runtime event subscription/unsubscription through graph-built context ports, system-resume preview rebind
   ...
   reinitialize facade, with graph-built context ports for preview state/events,
   capture/session operations, source telemetry refresh, UI dispatch,
   audio-preview activity, and preview-volume ramp-down. Sibling ViewModel controllers receive that preview
