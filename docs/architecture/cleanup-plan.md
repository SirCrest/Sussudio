...
registration file has been retired.
...
Whole-window screenshot automation stays on `MainWindow.xaml.cs` with the
...
`MainWindow.xaml.cs` is the `IAutomationWindowControl` adapter.
...
`Sussudio/MainWindow.xaml.cs` owns the stable automation
...
in `Sussudio/Controllers/Shell/ShellChromeController.cs`; `MainWindow.xaml.cs`
...
`Sussudio/MainWindow.xaml.cs` is the XAML/AppWindow close adapter and
...
Native XU runtime ownership now spans
`Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs` and
`Sussudio/Services/Telemetry/NativeXuAtProtocol.cs`. Keep public `ReadAsync`,
selected-interface reading, active rolling polling, XU transport, selector-4
I2C writes, scalar parsing, reference/full and rolling snapshot assembly,
diagnostic summary formatting, source telemetry detail rows, flash-audio input
interpretation, analog-gain detail insertion, audio-origin policy, generic
public AT SET/read wrappers, public `SwitchAudioInputAsync` and
`SetAnalogGainAsync` entry points, the HDMI/Analog codec switch sequence, and
analog gain register mapping/writes in the provider. Keep pure AT frame/LRC and
envelope handling plus AVI/HDR InfoFrame decoding in the protocol helper.
`tools/NativeXuAudioProbe` links both files explicitly, so update its project
file whenever either owner or its shared contracts change.
...
`Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs`; raw AT frame
construction, LRC, and response-envelope stripping live in
`Sussudio/Services/Telemetry/NativeXuAtProtocol.cs`.
...
command batch construction/refresh, per-command cancellation checks, selector-4
I2C payload writes, scalar helpers, and command failure formatting there with
the node-read path that calls them. The provider calls
`NativeXuAtProtocol.cs` for raw AT frames, LRC/envelope handling, and AVI/HDR
InfoFrame decoding.
...
Native XU AVI and HDR InfoFrame decoding now lives with frame/LRC/envelope
handling in `Sussudio/Services/Telemetry/NativeXuAtProtocol.cs`. Keep
scalar/ascii payload reads, frame-rate rational inference, confidence scoring,
and boolean token helpers in `NativeXuAtCommandProvider.cs` with the snapshot
...
`Sussudio/Services/Preview/D3D11PreviewRenderer.cs`. Keep present
...
live with the partial that mutates or projects them: keep renderer diagnostic
ring/write state, render-thread failure state, first-frame notification state,
slow-frame reason classification, startup/disposal lifecycle state,
...
pending-frame render dispatch, final render-thread drain, renderer-mode reset,
queue state/signaling, swap-chain panel binding state, and render-thread
waitable frame-latency pacing in `D3D11PreviewRenderer.cs`; keep D3D
device/swap-chain, input texture, HDR shader input resources, shader
resource/cache state, and shader compilation in
`D3D11PreviewRenderer.Resources.cs`; keep render-pass selection plus
VideoProcessor, NV12 shader, HDR shader execution, and shared present
accounting in `D3D11PreviewRenderer.RenderPasses.cs`.
...
`Sussudio/Services/Preview/D3D11PreviewRenderer.cs` beside metric
...
and `DwmFlush` in `D3D11PreviewRenderer.cs`; leave `WaitForSingleObject` with render-thread frame pacing in
...
`D3D11PreviewRenderer.cs`, render-pass selection,
...
lives in `Sussudio/Services/Preview/D3D11PreviewRenderer.cs`. Keep
...
`Sussudio/Services/Preview/D3D11PreviewRenderer.cs`. Keep
slow-frame diagnostic consumption of the latest DXGI counters in the same
renderer root.
...
Native XU AT-command transport and policy live in
`Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs`; pure framing and
InfoFrame decoding live in `NativeXuAtProtocol.cs`. Keep selector-4 I2C payload
writes, device-ID and scalar parsing, command failure formatting, rolling
telemetry polling, the active read path, and shared source snapshot assembly in
the provider. Keep raw AT read/write frames, LRC/envelope handling, and AVI/HDR
InfoFrame decoding in the protocol helper.
...
`Sussudio/Controllers/Stats/StatsOverlayCompositionController.cs`.
`Sussudio/MainWindow.xaml.cs` owns the XAML/automation
...
presentation ownership, source telemetry panel projection checks, and executable
stats presentation formatting behavior.
...
`Sussudio/MainWindow.xaml.cs` owns the XAML-facing compact
...
`tests/Sussudio.Tests/XUnit.ModelContractsTests.cs` for builder ownership,
source telemetry, stats dock, row chrome, frame-time overlay policy,
...
`Sussudio/Controllers/Stats/StatsOverlayCompositionController.cs`;
`Sussudio/MainWindow.xaml.cs` is the XAML-facing provider
composition adapter.
...
`Sussudio/Controllers/Preview/PreviewLifecycleControllers.cs`.
`Sussudio/MainWindow.xaml.cs` wires preview-transition
...
`Sussudio/MainWindow.xaml.cs` keeps the XAML event
...
chrome. `MainWindow.xaml.cs` is the XAML-facing adapter, while
...
`Sussudio/MainWindow.xaml.cs`. Timeout reason,
...
`Sussudio/MainWindow.xaml.cs`. Watchdog/timeout recovery remains in
`Sussudio/Controllers/Preview/Startup/PreviewStartupControllers.cs`.
Preview startup loading overlay presentation now lives in
`Sussudio/Controllers/Preview/PreviewLifecycleControllers.cs`.
`Sussudio/MainWindow.xaml.cs` is the XAML-facing adapter; watchdog and
...
`Sussudio/MainWindow.xaml.cs` wires renderer-host context
...
`Sussudio/MainWindow.xaml.cs` is the XAML-facing adapter and
...
`Sussudio/MainWindow.xaml.cs`, and live
