...
Notes for future agents: keep UnifiedVideoCapture source-session initialization/start/stop/dispose and MJPEG pipeline lifecycle in `Sussudio/Services/Capture/UnifiedVideoCapture.Lifecycle.cs`; keep frame arrival routing in `UnifiedVideoCapture.FrameIngress.cs`, sink queue fan-out in `UnifiedVideoCapture.SinkFanout.cs`, preview submission in `UnifiedVideoCapture.Preview.cs`, and metric projection/root state in `UnifiedVideoCapture.cs`.
