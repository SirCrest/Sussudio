...
Notes for future agents: keep Parallel MJPEG pipeline construction, compressed input admission, worker decode-loop execution, packet-hash metrics, pipeline timing projection, stop/dispose/shutdown, fatal callback signaling, and final resource cleanup in `Sussudio/Services/Gpu/ParallelMjpegDecodePipeline.cs`; keep decoded-frame ordering/emission in `ParallelMjpegDecodePipeline.Reorder.cs`.
...
Behavior preserved: construction/startup, compressed admission, startup invalid-MJPG drop, byte-budget/depth accounting, worker decode loops, packet-hash/timing metrics, emitter startup/signaling, stop/dispose, shutdown joins, fatal callback dispatch, cleanup of decoders/work items/reorder frames/emit signal, and remaining-time calculations now live in `ParallelMjpegDecodePipeline.cs`; decoded-frame ordering/emission remains in `ParallelMjpegDecodePipeline.Reorder.cs`.
Notes for future agents: keep Parallel MJPEG pipeline construction, compressed admission, worker decode-loop execution, packet-hash metrics, pipeline timing projection, stop/dispose, worker/emitter shutdown joins, emitter signaling, fatal callback dispatch, remaining-time calculations, decoder disposal, queued work item return, remaining reorder-frame disposal, and emit-signal disposal in `Sussudio/Services/Gpu/ParallelMjpegDecodePipeline.cs`; keep decoded-frame ordering, missing-sequence handling, emission, preview notification, and reorder latency in `ParallelMjpegDecodePipeline.Reorder.cs`.
