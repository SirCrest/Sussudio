...
Notes for future agents: keep Native XU transport plus payload decoding together in `Sussudio/Services/Telemetry/NativeXuAtCommandProvider.AtProtocol.cs`; keep public device/audio command entry points plus their shared gate/open/topology traversal and audio sequence helpers in `NativeXuAtCommandProvider.DeviceCommands.cs`; keep active rolling telemetry in `NativeXuAtCommandProvider.RollingPoll.cs`, snapshot assembly in `NativeXuAtCommandProvider.SnapshotAssembly.cs`, and detail row formatting in `NativeXuAtCommandProvider.TelemetryDetails.cs`.
