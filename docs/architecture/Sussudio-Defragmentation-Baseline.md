...
Notes for future agents: keep CaptureService public recording start routing, standard LibAv recording startup, context request assembly, rollback-state holder, and failed-start cleanup together in `Sussudio/Services/Capture/CaptureService.RecordingLifecycle.cs`; keep Flashback recording backend startup in `CaptureService.FlashbackRecording.cs` and backend-specific finalization in the focused `RecordingFinalize*Backend.cs` owners.
