...
Notes for future agents: superseded by later MJPEG jitter ingress locality slices; keep MJPEG preview jitter construction, emit loop, suppression/reprime lifecycle, disposal, metrics projection, decoded-frame queue ingress, queue ordering/dequeue selection, and pooled payload ownership in `Sussudio/Services/Capture/MjpegPreviewJitterBuffer.cs`; keep display-clock alignment, preview submission, deadline drops, and adaptive target-depth policy in `MjpegPreviewJitterBuffer.FramePacing.cs`.
...
Behavior preserved: construction, emit loop, suppression/reprime, dispose, metrics, buffered-frame ownership, raw/lease enqueue, queue ordering/dequeue selection, clearing, and reprime miss accounting now live in `MjpegPreviewJitterBuffer.cs`; display-clock alignment, submission, deadline drops, and adaptive target-depth policy remain in `MjpegPreviewJitterBuffer.FramePacing.cs`.
Notes for future agents: keep MJPEG preview jitter root lifecycle, decoded-frame queue ingress, pooled payload ownership, queue ordering/dequeue selection, clear behavior, reprime recovery, and metric projection in `Sussudio/Services/Capture/MjpegPreviewJitterBuffer.cs`; keep display-clock alignment, renderer submission, deadline/adaptive depth policy, tick waits, and timer-resolution P/Invoke in `MjpegPreviewJitterBuffer.FramePacing.cs`.
