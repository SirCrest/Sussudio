...
Notes for future agents: keep CaptureService read-only diagnostics, automation probes, source telemetry polling/fallback, observed pixel telemetry, recording stats, and A/V sync helper policy together in `Sussudio/Services/Capture/CaptureService.Snapshots.cs`; keep runtime snapshot sampling in `CaptureService.RuntimeSnapshots.cs`, final runtime DTO assembly in `CaptureService.RuntimeSnapshotAssembler.cs`, health snapshot sampling in `CaptureService.HealthSnapshots.cs`, and final health DTO assembly in `CaptureService.HealthSnapshotAssembler.cs`.
