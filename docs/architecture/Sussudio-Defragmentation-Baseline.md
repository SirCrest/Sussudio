...
Notes for future agents: keep decoder state guards, error helpers, and validation helpers with `FlashbackDecoder.cs`; keep playback timing, seeking, packet feed, and inline audio output in `FlashbackDecoder.Playback.cs`; keep codec setup in `VideoSetup.cs` and video output conversion in `VideoOutput.cs`.
...
Notes for future agents: keep Flashback decoder seek flow, video packet feed, inline audio delivery/conversion, audio codec/resampler setup, and decode phase timing together in `Sussudio/Services/Flashback/FlashbackDecoder.Playback.cs`; keep video codec setup in `FlashbackDecoder.VideoSetup.cs`, decoded video frame output/conversion in `FlashbackDecoder.VideoOutput.cs`, and root lifecycle/open/close/dispose plus validation helpers in `FlashbackDecoder.cs`.
