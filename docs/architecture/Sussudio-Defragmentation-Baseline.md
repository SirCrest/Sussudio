...
Notes for future agents: superseded by the 2026-05-30 unified capture ingress/fan-out locality slice; keep UnifiedVideoCapture source-session initialization/start/stop/dispose and MJPEG pipeline lifecycle in `Sussudio/Services/Capture/UnifiedVideoCapture.cs`; keep frame arrival, preview submission, and sink queue fan-out in `UnifiedVideoCapture.FrameIngress.cs`; keep metric projection/root state in `UnifiedVideoCapture.cs`.
...
Notes for future agents: superseded by the 2026-05-30 unified capture ingress/fan-out locality slice; keep UnifiedVideoCapture frame arrival, preview routing, visual-cadence sampling, pixel-format observer dispatch, fatal-error signaling, and recording/Flashback sink fan-out together in `Sussudio/Services/Capture/UnifiedVideoCapture.FrameIngress.cs`; keep source-session lifecycle in `UnifiedVideoCapture.cs`.
...
Notes for future agents: superseded by the 2026-05-30 unified capture ingress/fan-out locality slice; keep UnifiedVideoCapture root capture fields, public control/config surface, recording/Flashback attachment state, source-session initialization/start/stop/dispose, CPU MJPEG pipeline lifecycle, source-reader cadence forwarding, MJPEG timing/jitter/hash metrics, visual cadence metrics, and frame-ledger projection in `Sussudio/Services/Capture/UnifiedVideoCapture.cs`; keep frame arrival, preview ingress, recording fan-out, and Flashback fan-out in `UnifiedVideoCapture.FrameIngress.cs`.
...
Notes for future agents: keep UnifiedVideoCapture source-frame arrival, MJPEG emission, preview routing, recording enqueue, Flashback enqueue, frame-ledger accounting, pixel-format observer dispatch, visual cadence sampling, and fatal-error signaling in `Sussudio/Services/Capture/UnifiedVideoCapture.FrameIngress.cs`; keep source-session lifecycle and metrics projection in `UnifiedVideoCapture.cs`.
