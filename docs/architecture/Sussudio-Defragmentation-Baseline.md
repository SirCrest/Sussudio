...
Notes for future agents: superseded by later unified capture locality slices; keep UnifiedVideoCapture source-session initialization/start/stop/dispose and MJPEG pipeline lifecycle in `Sussudio/Services/Capture/UnifiedVideoCapture.cs`, frame arrival and preview submission in `UnifiedVideoCapture.FrameIngress.cs`, sink queue fan-out in `UnifiedVideoCapture.SinkFanout.cs`, and metric projection/root state in `UnifiedVideoCapture.cs`.
