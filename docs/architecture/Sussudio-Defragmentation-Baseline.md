...
Notes for future agents: keep CaptureService public recording start routing, standard LibAv recording startup, context request assembly, rollback-state holder, and failed-start cleanup together in `Sussudio/Services/Capture/CaptureService.RecordingLifecycle.cs`; keep the Flashback recording backend lifecycle in `CaptureService.FlashbackRecording.cs` and standard LibAv finalization in `CaptureService.RecordingFinalizeLibAvBackend.cs`.
...
Notes for future agents: keep the Flashback recording backend lifecycle in `Sussudio/Services/Capture/CaptureService.FlashbackRecording.cs`; keep standard LibAv recording finalization in `CaptureService.RecordingFinalizeLibAvBackend.cs` and public start/stop routing plus outcome field publication in `CaptureService.RecordingLifecycle.cs`.
