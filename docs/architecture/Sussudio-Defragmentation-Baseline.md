...
Notes for future agents: keep Flashback sink construction, root state, start/rollback, runtime projection, recording lifecycle, stop, and dispose in `Sussudio/Services/Flashback/FlashbackEncoderSink.cs`; keep producer queues and packet ownership in `FlashbackEncoderSink.Queueing.cs`, background drains in `FlashbackEncoderSink.EncodingLoop.cs`, and export force-rotation in `FlashbackEncoderSink.ForceRotate.cs`.
