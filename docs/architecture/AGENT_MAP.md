...
| Device audio control | `Sussudio/Services/Audio/NativeXuAudioControlService.cs` | direct Native XU selector-3 audio-control read/modify/write service for 4K X devices, HDMI/Analog reference payloads, dynamic/control byte preservation, transport-gated KS candidate scanning, payload normalization/rehydration, mode/gain state readback, and diagnostic payload snapshot DTOs |
| Device audio mode validation | `Sussudio/Models/DeviceAudioModeParser.cs` | strict case-insensitive parsing of the two supported automation mutation values, canonical HDMI/Analog spelling, and rejection before UI, native-device, or settings mutation |
| Capture source reader | `Sussudio/Services/Capture/MfSourceReaderVideoCapture.cs`, `MfInterop.cs` | consolidated source-reader state, public counters, GPU-native MJPG-to-NV12 capability state, shared packed-frame sizing/stride/subtype helpers, initialization validation/orchestration, reader construction, direct device opening, device-enumeration open fallback and candidate reporting, native media-type selection, converted output media-type construction, actual-output reconciliation, initialized runtime-state commit, active reader start/stop/dispose plus Media Foundation read loop and source cadence metrics, sample-to-frame dispatch, compressed MJPG extraction, raw CPU frame extraction, 2D buffer handling, packed-stride CPU copies, DXGI texture extraction, dual GPU/CPU delivery orchestration, debug-only COM diagnostics, and general Media Foundation COM interface definitions plus MF startup/attribute helpers, top-level P/Invoke/constants/GUID holders, and flattened sample/buffer COM interface definitions |
| Capture fan-out | `Sussudio/Services/Capture/UnifiedVideoCapture.cs` | public control/config surface, source-reader/D3D/MJPEG initialization and state commit, capability-gated GPU-native MJPEG initialization with hardware-free fallback orchestration policy and explicit software fallback, shared source-reader start/stop/dispose lifecycle plus CPU MJPEG stop/dispose/fatal handling, source-reader frame ingress, preview submission, visual-cadence handling, fatal-error signaling, recording and Flashback sink queue fan-out, and diagnostic metric/snapshot projection |
...
| WASAPI interop | `Sussudio/Services/Audio/WasapiComInterop.cs` | native constants/P/Invokes, COM release helpers, format allocation/parsing including container bits, valid bits, block alignment, and channel masks, device enumerator and endpoint volume helpers, debounced endpoint-change watcher, AudioClient activation/AudioClient3 initialization, shared audio format/PROPVARIANT structs, Core Audio device/property contracts, and AudioClient/capture/render/endpoint-volume contracts |
| WASAPI worker quarantine | `Sussudio/Services/Audio/WasapiWorkerQuarantine.cs` | role-scoped retention of capture/playback workers that miss the five-second terminal boundary, restart blocking while worker-reachable native resources remain live, late-exit reaping without caller-side COM/event release, and the emergency-close notification consumed by the still-live MainWindow acknowledgment/countdown path |
...
| MJPEG decode pipeline | `Sussudio/Services/Gpu/ParallelMjpegDecodePipeline.cs` | pipeline construction/startup sequencing, bounded work-channel construction, compressed input admission/byte budget/depth accounting, CPU MJPEG worker decode-loop execution and decoder ownership, pipeline timing and packet-hash metrics, stop/dispose/shutdown joins/fatal callback signaling, decoder/work-item/reorder-frame resource cleanup, decoded-frame ordering, missing-sequence state, decoded-frame emission and preview notification, software MJPEG decoder initialization/lifetime, decode/copy hot path, and error text. The prior unused CUDA/D3D11 bridge and NVDEC decoder files are intentionally absent until live pipeline callers own those paths. |
...
| FFmpeg D3D11 ownership | `Sussudio/Services/Gpu/FfmpegD3D11Ownership.cs` | atomic two-reference D3D11 device/immediate-context retention and transfer into `AVD3D11VADeviceContext`, rollback before publication, and final-release delegation to the enclosing FFmpeg `AVBufferRef` |
| Automation diagnostics | `Sussudio/Services/Automation/AutomationDiagnosticsHub.cs`, `AutomationDiagnosticsHub.Evaluation.cs`, `AutomationDiagnosticsHub.Snapshots.cs`, `AutomationDiagnosticsHub.SnapshotProjection.cs`, `AutomationSnapshotFlashbackProjectionBuilder.cs` | additional collectors/controllers when hub orchestration grows |
...
| Source telemetry | `Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs`, `NativeXuAtProtocol.cs` | ReadAsync validation/gating through shared Native XU device support, selected-interface open/topology/node scan, node-read failure classification, active rolling poll cadence/cache plus rolling command group dispatch and per-command cancellation helpers, XU transport and selector-4 I2C payload writes, pure AT frame/LRC/envelope construction plus AVI/HDR InfoFrame decoding, source scalar and snapshot-policy helpers, public HDMI/Analog audio route and gain command entry points plus HDMI/Analog switch sequence, analog gain register mapping and writes, generic public SET-command surface and generic public read-only AT-command surface, reference full-snapshot command acquisition, full/rolling AT-command handoff contract, VIC/frame-rate lookup policy, source snapshot assembly from AT-command results, diagnostic summary formatting, source telemetry detail row assembly, flash-audio input, analog-gain detail interpretation, audio-origin policy, and AT detail value formatting |
| App service contracts | `Sussudio/Services/Contracts/ServiceContracts.cs`, `Sussudio/Services/Contracts/ISourceSignalTelemetryProvider.cs` | shared in-process app-service contracts and pooled-frame ownership types; `ServiceContracts.cs` owns automation window/diagnostics/command-dispatcher interfaces, preview sink and `PreviewFrameTracking`, recording context/finalize DTOs including requested-versus-observed track evidence, raw/lease/GPU/CUDA frame encoder contracts, and the reference-counted pooled-frame/lease pair; `ISourceSignalTelemetryProvider.cs` remains the probe-linked source signal telemetry provider plus `Sussudio.Models` source telemetry DTO/enums/detail rows linked by `NativeXuAudioProbe`; keep these separate from `Sussudio.Automation.Contracts` wire/protocol contracts |
| Recording | `Sussudio/Services/Recording/LibAvEncoder.cs`, `LibAvEncoder.Audio.cs`, `LibAvEncoder.VideoFrames.cs`, `LibAvRecordingSink.cs`, `Sussudio/Services/Recording/Verification/RecordingVerifier.cs`, `InProcessRecordingStructureVerifier.cs` | encoder core state plus option/result DTOs, open/error/device-removed diagnostics, encoder runtime/open initialization including option validation, video codec setup, codec/filter policy, NVENC preset/split-encode mapping, rational conversion, frame-size math, and audio sample-format support, output rotation/reopen/reset, mux phase timing, muxer option policy, failure-propagating final close/trailer, and native resource release/reset, audio/microphone stream state, public audio submission, audio and microphone AAC stream initialization, sample queue/drain mechanics, A/V sync diagnostics, and audio packet writes, video packet drain/write helpers, CPU packed-frame submission, packed software-frame copy helpers, HDR side-data attachment/parsing for submitted frames, D3D11/CUDA hardware-frame setup and submission, sink state/construction, read-only diagnostics surface, recording sink startup shell, encoder option creation, video/GPU/CUDA session queue initialization, metric reset, video diagnostics reset, encode loop orchestration, video/GPU/CUDA/audio/microphone packet drains, worker-owned dispose/deferred cleanup, 30-second no-progress and 120-second absolute finalization policy, recovery preservation, in-process libav reopen/topology/metadata verification as save authority, bounded HDR validation, recording sink video/GPU/CUDA public enqueue adapters, hot audio/microphone write adapters, queue admission/cleanup, pooled packet ownership, shared video queue latency/backpressure tracking, and optional external ffprobe diagnostic verification |
| Portable release tooling | `tools/reliability-gates.ps1`, `tools/package-github-release.ps1`, `tools/release/release-helpers.ps1`, `Sussudio/ffmpeg/manifest.json` | canonical solution/tool build plus nonzero xUnit/TRX/offline-harness/diff gate, clean exact SemVer-prerelease tag enforcement, deterministic app/tool ZIP inventory, pinned third-party FFmpeg identity, external Azure Artifact Signing prerequisites, explicit first-party signing/verification, and final ZIP SHA-256 generation |
| Flashback | `FlashbackDecoder.cs`, `FlashbackPlaybackController.cs`, `FlashbackPlaybackCommandMailbox.cs`, `FlashbackPlaybackController.PlaybackFrames.cs`, `FlashbackPlaybackController.ThreadCommands.cs`, `FlashbackEncoderSink.cs`, `FlashbackBufferManager.cs`, `FlashbackStartupCacheCleanup.cs`, `FlashbackExporter.cs`, `FlashbackExportOutputTransaction.cs`, `FlashbackExportPlanner.cs` | consolidated decoder lifecycle/open/dispose shell, state guards, FFmpeg error formatting, decoded stream/frame validation helpers, decoded video/audio output DTOs, video codec setup, D3D11 device-context initialization, D3D11VA/software fallback selection, hardware decoder context setup, D3D11VA decoder discovery, hardware-config diagnostics, frame-rate metadata, MJPEG decode policy, software output-buffer allocation, decoded video frame output, hardware/software frame selection, decoded PTS helpers, software plane copy/conversion kernels, decoder seek conversion helpers, keyframe/exact seek control flow, video frame receive, packet feed loop, inline audio packet delivery, audio codec/resampler initialization, callback failure handling, bounded audio output, recoverable seek log suppression, decode phase timing accumulation, decoder file-close native cleanup, and held-frame release, playback core, decoder file open/identity, decoder cleanup/close telemetry, active fMP4 reopen, seek recovery, and adjacent-segment seek fallback, segment-edge fMP4 reopen and audio gate recovery, component lifecycle, dispose, preview-detach deferred reattach lifecycle, consolidated playback metrics and seek-cap telemetry, public playback command facade, bounded per-generation command mailbox with queue/drop/coalescing/yield/telemetry ownership, consolidated playback-thread lifecycle, playback thread command dequeue/wait loop, playback-thread command dispatch and completion telemetry, playback-thread seek/scrub begin/update command execution, playback-thread end-scrub resume execution, playback-thread play command execution, playback-thread pause command execution, playback-thread nudge command execution, terminal go-live/stop dispatch execution, audio callback/routing/render helpers, audio prebuffer/rewind, audio-master clock/A/V drift projection, audio-master pacing/fallback projections, decoded frame preview submission and validation/byte sizing, seek/scrub keyframe display, seek/scrub decoded-frame display handoff, live playback recovery, continuous playback loop, segment-edge routing/write-head handling, next-segment switch transaction, timing policy, decoded PTS cadence state/telemetry/projection, software-decode budget snap policy, marker command, state, file-PTS, and range owner, position/file-PTS mapping, consolidated Flashback encoder core state/construction, startup transaction, startup queue construction, startup failure rollback, startup validation, queue-capacity policy, startup metric/counter reset, video diagnostics reset, encode loop orchestration, video/GPU packet drains, audio/microphone packet drains, encoded-frame progress publication plus segment rotation/failure recovery, export force-rotation status/idle projection, request admission/result classification, lifecycle cleanup, request state machine, encoding-thread force-rotate execution, video/GPU/audio/microphone producer input surface, lifecycle/input guards, queue admission, TryWrite depth accounting, rejection telemetry, hot WASAPI writer adapters, stop/dispose lifecycle, encoder option/session construction and recording-to-Flashback mapping, file/session helpers, shared encoder queue helpers, packet DTOs, pooled packet buffer ownership, video enqueue result classification, best-effort video packet cleanup, GPU texture release helpers, queued-buffer cleanup, recording state/gates, start/rollback, finalization, public runtime counters, public queue telemetry, public encoder status/format projections, saturated PTS conversion, non-negative byte/duration math, and best-effort eviction resume fallback, buffer core state, shared buffer math and saturated accounting helpers, buffer live byte/PTS accounting updates, buffer initialize/dispose lifecycle, buffer recovery-preserve markers, purge/delete-all cleanup, guarded purge file deletion, buffer segment mutation surface, buffer segment completion/extension, buffer segment query helpers, buffer segment path safety, buffer segment status/projection helpers, buffer retention/eviction, buffer eviction-pause state and recording PTS range capture, startup stale-root/session cleanup, free-space probing, and startup session-cache budget enforcement, exporter lifecycle/disposal/native state/native cleanup/lock handling, input/output stream setup, stream-template/layout compatibility, single-file packet result validation, single-file active input packet pump, multi-segment export lifecycle/template/preflight, multi-segment packet writing/remux orchestration, active segment packet pump/write state/outcomes plus segment timestamp rebasing/native writes, packet timestamp helpers, packet buffer lifetime helpers, segment export range/window projection, segment validation policy, output/path/range validation, export request routing/scheduling, single-file export shell, export runtime progress/pacing policy, stream/context setup, stream-template/layout compatibility, export failure results, export validation, FFmpeg error formatting, and output-transaction reservation/file-identity/safe-cleanup/no-replace publication. `FlashbackExportPlanner.cs` owns pure post-eviction range resolution, force-rotate outcome selection, segment metadata mapping/PTS repair, and request skeleton construction from immutable snapshots; CaptureService retains locks, live buffer/sink/filesystem sampling, path normalization, diagnostics, fallback counters, throttle policy, and exporter dispatch. |
...
| Preview rendering | `D3D11PreviewRenderer.cs`, `D3D11PreviewRenderer.RenderPasses.cs`, `D3D11PreviewRenderer.Resources.cs`, `PreviewOutputSizePolicy.cs`, `PreviewScreenshotCapture.cs` | renderer public facade, construction, constants, env-tuned runtime configuration, render-thread startup/disposal state, public stop/reinit-stop lifecycle, user-facing state accessors, native SwapChainPanel bind/unbind interop, panel size/transform state, stable visible-panel output-size bucketing, resize hysteresis/debounce, render-thread-only swap-chain resize telemetry, stop/unbind/native-call fence lifecycle, render-thread loop/orchestration plus shared-device reset consumption/rebind, frame-latency waitable swap-chain state/setup/waits, composition-transform wake handling, public raw/lease/single-texture frame submission entry points, dual-plane NV12 submission, HDR transition telemetry, pending-frame lifetime, queue/signaling state, queued-frame render dispatch, pending-frame consumption, explicit queue-drain control, final render-thread drain/state reset, renderer diagnostics, render-thread failure telemetry state, first-frame notification state, DXGI frame statistics state, optional `DwmFlush` interop, display-clock projection, read-only present-cadence metrics state/projection, read-only latency/render/wait metrics state/projection, submitted/rendered/dropped frame ownership state and telemetry, expected-frame-rate metric window sizing/reset, renderer metric model types and shared metric sample helpers, render-loop metric window tracking, slow-frame ring/projection/reason classification and DXGI refresh-slip capture, render-pass selection plus VideoProcessor, NV12 shader, and HDR shader execution, VideoProcessor input-view resolution, external-texture input-view helpers, raw frame direct/staging upload helpers, viewport/letterbox helpers, shared present/accounting transaction, screenshot capture request/result/PNG completion lifecycle, GPU/readback before present, and staging reuse/teardown, shader state, cached shader resources, shader compilation resources, HLSL shader sources, renderer mode labels, and D3DCompiler blob interop, shared source-reader D3D manager and `ILiveVideoSource` handoff contract, shared-device COM reference handoff, reinit retirement, reset scheduling, device-lost classification/recovery, D3D device/swap-chain initialization, D3D object fields, input texture resources, HDR shader input resources, top-level cleanup orchestration, video-processor pipeline setup/teardown, output-view/RTV reuse, and VideoProcessor color-space updates, preview BMP/PNG pixel analysis and file encoding, timing models |
...
| Presentation | `MainViewModel.*.cs` facade/feature partial family, `Sussudio/ViewModels/MainViewModel.cs`, plus focused `Sussudio/ViewModels` policy/presentation helpers | keep the root facade stable while moving pure feature state, controller graph construction, policy, and presentation logic into named owners |

### Flashback export output transaction

- `Sussudio/Services/Flashback/FlashbackExportOutputTransaction.cs` owns each
  export's reserved temporary file, file-identity verification, safe failure or
  cancellation cleanup, stale `.mp4.tmp` cleanup in the Flashback temp root,
  and no-replace handle-based publication. `FlashbackExporter.cs` owns FFmpeg
  trailer writing and output-IO closure, then hands the completed temporary file
  to that transaction for validation and publication.

## Major Runtime Closure Status

These classifications are the current closure guide for the remaining large
runtime systems. Reclassify only from live source, tests, and baseline evidence;
do not use these rows as permission for cosmetic file-count churn.
...
| AutomationDiagnosticsHub | Ready | `Sussudio/Services/Automation/AutomationDiagnosticsHub.SnapshotProjection.cs` still owns root projection orchestration and the final `AutomationSnapshot` wire initializer, while `Sussudio/Services/Automation/AutomationSnapshotFlashbackProjectionBuilder.cs` now owns Flashback export, recording, playback, and flattening projection construction. `Sussudio/Services/Automation/AutomationDiagnosticsHub.cs`, `Sussudio/Services/Automation/AutomationDiagnosticsHub.Snapshots.cs`, and `Sussudio/Services/Automation/AutomationDiagnosticsHub.Evaluation.cs` remain the polling/timeline, stateful refresh/alerts, and diagnostic verdict owners. The current `tests/Sussudio.Tests/ArchitectureGuardrails.Tests.cs` and `tests/Sussudio.Tests/XUnit.AutomationContractsTests.cs` coverage protects wire fields, classifier wiring, and the builder boundary. | Treat the current hub family as closed unless a future change can move another named projection collaborator without changing the final automation wire initializer or CLI/MCP/tool contracts. Keep snapshot mapping changes local to the hub/builder family and update source-shape tests with any moved projection surface. |
...
- `Sussudio/Services/Preview/D3D11PreviewRenderer.cs` owns the renderer public
  ...
  renderer mode reset. It also owns renderer metrics and diagnostics:
...
- `Sussudio/Services/Preview/D3D11PreviewRenderer.Resources.cs` owns D3D
  device/swap-chain/video-processor fields, shared source-reader D3D manager
  lifecycle plus the `ILiveVideoSource` handoff contract, `InitializeD3D` orchestration,
...
- `Sussudio/Services/Preview/D3D11PreviewRenderer.cs` owns native
  ...
  matrix updates.
- `Sussudio/Services/Preview/D3D11PreviewRenderer.cs` owns present
...
- `Sussudio/Services/Automation/AutomationDiagnosticsHub.SnapshotProjection.cs`
  ...
  owns the former capture-format, media, and preview projection shards while
  they remain private projection records over the same snapshot composition and
  flattening path. Keep this
...
- `CaptureService.PreviewLifecycle.cs` owns preview volume/mute commands,
  ...
  checks.
- `CaptureService.cs` owns the capture service resource holders:
...
- `CaptureService.Flashback.cs` owns Flashback export entry points, lock-scoped
  ...
  shared export lifetime, export-operation locking, eviction pause/resume,
  live force-rotate and stable-path sampling, path normalization, active-file
  sampling, diagnostics/logging, fallback counters, throttle-provider wiring,
  exporter dispatch, partial-fallback result marking, and cleanup.
  `FlashbackExportPlanner.cs` owns pure post-eviction range and last-N
  resolution, buffer-position clamps, saturating PTS math, force-rotate outcome
  selection, request skeleton construction, segment metadata mapping, and PTS
  repair from immutable snapshots. Keep the planner free of buffer, sink,
  filesystem, lock, clock, and FFmpeg ownership. `CaptureService.Flashback.cs`
  also owns export result/rejection diagnostic
...
- `CaptureService.cs` also owns `CaptureVideoPipelineResources`:
...
- `CaptureService.RuntimeSnapshots.cs` owns read-only automation probes,
  preview-frame capture waits, diagnostics compatibility snapshot helpers, and
  the shared snapshot helper policy consumed by runtime and health readback.
...
- `CaptureService.RecordingLifecycle.cs` owns standard LibAv
  ...
  summaries, recording integrity active-backend resolution, private video/audio
  counter DTOs, baseline deltas, final `RecordingIntegritySummary`
  construction, status/reason/audio-status classification, structured
  `RECORDING_INTEGRITY` logging, final state completion, pending Flashback enable-after-recording
...
- `CaptureService.RuntimeSnapshots.cs` also owns diagnostics-snapshot
...
- Flashback recording stop uses accepted/retired per-queue fences and a latched
  ...
  before nonessential preview maintenance, and keeps that maintenance visible
  as cleanup-pending so recording cannot re-enter early.
...
- `FlashbackDecoder.cs` owns decoder lifecycle, file open/close, dispose shell,
  ...
  bounded audio output, live-file EOF clearing, recoverable seek log
  suppression, decode phase timing state, video codec setup, D3D11
...
- `FlashbackPlaybackController*.cs` plus `FlashbackPlaybackCommandMailbox.cs` own playback, scrub, and marker control.
- `FlashbackPlaybackController.cs` owns decoder creation, active file
...
- `FlashbackPlaybackController.cs` owns construction, the `FlashbackBufferManager`
  ...
  disposal, state-transition logging, the playback-thread command payload
  contract, public playback command entry points for scrub, seek, play/pause,
  go-live, and nudge, public command/thread metrics, command readiness/failure
...
- `FlashbackPlaybackController.ThreadCommands.cs` owns playback-thread
  ...
  command mailbox generation lifetime, scheduling policy, exit transactions, live-restore
...
- `FlashbackPlaybackController.cs` owns live audio
...
- `FlashbackExportOutputTransaction.cs` owns one export's filesystem lifetime:
  ...
  closes the native output before handing the file to this collaborator.
...
- `Sussudio/Controllers/Window/WindowControllers.cs` owns window close
  ...
  effects, and post-`Closed` cleanup order: `Close()`, pre-close ViewModel/audio
  disposal while the quarantine UI remains available, completion timing after
  safe non-recording closes, close-in-progress success handling, COM
...
- `Sussudio/MainWindow.xaml.cs` owns the XAML/AppWindow close adapter:
  ...
  `RequestWindowClose()` adapter, plus the quarantined-WASAPI explanation,
  emergency recording cleanup attempt, acknowledgment, ten-second countdown,
  and bounded process-exit path.
- `Sussudio/MainWindow.xaml.cs` wires MainWindow cleanup
  ...
  adapters.
- `Sussudio/Controllers/Window/WindowControllers.cs` also owns native window
...
- `tests/Sussudio.Tests/XUnit.ModelContractsTests.cs` owns model and presentation
  contract checks, including stats presentation formatting plus stats dock
  refresh orchestration, diagnostic row update delegation, hardware row
  projection, source-shape ownership, HDMI source telemetry panels, and row
  chrome pooling.
...
- `tests/Sussudio.Tests/XUnit.PresentationPreviewContractsTests.cs` owns D3D11
  ...
  diagnostic reflection contracts, `AutomationModels.cs` preview,
  ...
  shader-rendering, shader-source, public submission state,
  lifecycle/stop-lifecycle state, pending-frame state and draining,
...
- `tests/Sussudio.Tests/HarnessCore.cs` owns the assembly-load smoke entry point
  and xUnit target-assembly bootstrap; regression checks live in focused xUnit
  slices and contract files rather than a second harness catalog.
...
- `Sussudio/ViewModels/MainViewModel.*.cs` for root presentation state and
  ...
  `tests/Sussudio.Tests/XUnit.ModelContractsTests.cs` owns executable coverage for
  those pure view-model DTO builders plus source telemetry and live-signal text
  presentation helpers through `ViewModelBuildersTests`.
  ...
  cleanup.
  `MainViewModel.cs` owns capture-device selection reactions,
  ...
  filtering with capture options always visible. `MainViewModel.cs`
  ...
  device-native audio-control refresh handoff live in `MainViewModel.cs`; capture-mode property-change hooks live
  in `MainViewModel.cs`; startup audio-list and
  ...
  `MainViewModel.cs` owns effective Source resolution state
  ...
  `MainViewModel.cs` owns settings initialization, simple
  ...
  `MainViewModelLifecycleController.cs` owns top-level automation preview
  ...
  `MainViewModel.cs` owns automation HDR and true-HDR
...
- `Sussudio/ViewModels/MainViewModel.cs` owns
