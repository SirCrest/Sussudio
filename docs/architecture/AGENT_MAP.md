...
- `FlashbackPlaybackController.CommandQueue.cs` owns public playback command
  ...
  metrics, command readiness/failure state, and no-op logging.
...
- `FlashbackPlaybackController.ThreadCommands.cs` owns playback-thread
  ...
  handoff.
- `FlashbackPlaybackController.ThreadSeekCommands.cs` owns playback-thread
  seek and scrub begin/update command execution, including coalesced seek/scrub
  resolution, exact resume targets, playback resume handoff, frozen valid-start
  sampling, scrub-display failure recovery, and audio/preview
...
- `FlashbackExporter.PacketTiming.cs` owns export time-span conversion,
  ...
  timestamp normalization, and segment boundary timestamp repair.
