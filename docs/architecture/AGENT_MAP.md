...
- `CaptureService.RecordingFinalizeFlashbackBackend.cs` owns active Flashback
  ...
  integrity summaries, pending-settings apply or buffer-cycle handling,
...
- `CaptureService.RecordingFinalizeLibAvBackend.cs` owns standard LibAv
  ...
  tracking, encoder/runtime and recording-integrity summaries, pending
  Flashback enable after recording, and standard post-recording microphone
  monitor restart.
...
- `tests/Sussudio.Tests/RecordingQueue.LibAvSink.Tests.cs` owns LibAv recording
  ...
  startup, stop-lifecycle, and lifetime-helper ownership assertions.
