...
| Automation snapshot models | `Sussudio/Models/Automation/AutomationSnapshot*.cs`, `AutomationCommandProtocol.cs`, `AutomationOptionsSnapshot.cs`, `CaptureRuntimeSnapshot*.cs`, `DiagnosticsEvents.cs`, `FlashbackSegmentInfo.cs`, `PerformanceTimelineEntry*.cs`, `PreviewRuntimeSnapshot*.cs`, `RecordingVerification.cs`, `VideoSourceProbe.cs`, `ViewModelRuntimeSnapshot.cs`, `WindowAutomation.cs` | automation evidence DTO partials by domain, command protocol DTOs, automation options DTO, capture runtime DTO partials by runtime surface, diagnostics events, Flashback segment DTOs, performance timeline entry partials by diagnostics surface, preview runtime DTO partials by diagnostics surface, recording verification DTOs, video source probe DTOs, view-model runtime DTO, window automation DTOs |
