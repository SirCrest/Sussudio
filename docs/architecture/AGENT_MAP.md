...
| Source telemetry | `Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs`, `NativeXuAtCommandProvider.AtProtocol.cs`, `NativeXuAtCommandProvider.DeviceCommands.cs`, `NativeXuAtCommandProvider.DiagnosticSummary.cs`, `NativeXuAtCommandProvider.FullSnapshot.cs`, `NativeXuAtCommandProvider.RollingPoll.cs`, `NativeXuAtCommandProvider.TelemetryDetails.cs` | Native XU selected-interface validation and ReadAsync orchestration, AT-command transport/parsing, public device-command surface, diagnostic summary formatting, reference full-snapshot reader, active rolling poll/snapshot assembly, source telemetry detail presentation |
