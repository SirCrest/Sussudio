...
- `tests/Sussudio.Tests/CaptureService.RuntimeSnapshots.Tests.cs` owns
  ...
  runtime projection ownership for ingest/audio, HDR pipeline, and recording
  integrity.
...
- `tests/Sussudio.Tests/CommandHandlers.Tests.cs` is the ssctl command-handler
  ...
  smoke coverage, and `CommandHandlers.Help.Tests.cs` owns ssctl help/catalog
  force-flag coverage.
