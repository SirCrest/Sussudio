...
- `FlashbackPlaybackController.ThreadStopCommand.cs` owns playback-thread stop
  command execution: playback/scrub state reset, exact-resume target clearing,
  live-restore handoff, and exit logging.
...
- `FlashbackPlaybackController.AudioMasterClock.cs` owns audio-master clock
  sample state, stale-clock detection, and drift projection.
...
- `FlashbackPlaybackController.Metrics.cs` owns playback diagnostic counters
  and the read-only A/V drift projection, excluding command/thread telemetry
  projection, audio-master projection, decoded-PTS cadence projection, max
  decode timing projection, and seek-cap telemetry projection.
...
- `FlashbackPlaybackController.MetricsCollection.cs` owns private playback
  metric counters and cadence sample rings.
