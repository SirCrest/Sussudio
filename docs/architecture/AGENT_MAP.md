...
- `Sussudio/Services/Automation/AutomationDiagnosticsHub.SignalAlerts.Preview.cs`
  owns preview blank, stall, startup, cadence, display 1% low, capture cadence
  drop, capture 1% low, audio muted signal, and recording output-growth signal
  alert rules.
...
- `Sussudio/Services/Automation/AutomationDiagnosticsHub.SnapshotProjection.PreviewD3D.cs`
  ...
  pipeline-latency projection consumed by `AutomationSnapshot`.
...
- `Sussudio/Services/Automation/AutomationDiagnosticsHub.TimelineProjection.FlashbackPlayback.cs`
  owns Flashback playback timeline projection composition, grouped handoff,
