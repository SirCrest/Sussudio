...
- `FlashbackPlaybackController.PlaybackDecodeMetricsCollection.cs` owns
  ...
  resolution.
- `FlashbackEncoderSink.Startup.cs` owns buffer session creation, encoder initialization, queue creation, background task startup, and startup rollback.
