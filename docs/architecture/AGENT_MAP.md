...
- `FlashbackPlaybackController.PlaybackPtsCadence.cs` owns decoded PTS cadence
  tracking, mismatch telemetry, and cadence-baseline reset.
...
- `FlashbackPlaybackController.MetricsCollection.cs` owns private metric state,
  cadence sample rings, and seek-cap telemetry.
