...
- `FlashbackPlaybackController.AudioMasterPacing.cs` owns audio-master pacing
  correction policy and wall-clock sleep/spin pacing.
...
- `FlashbackPlaybackController.Metrics.cs` owns playback diagnostic counters
  ...
  projection, decoded-PTS cadence projection, max decode timing projection, and
  seek-cap telemetry projection.
