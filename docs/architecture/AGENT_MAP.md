...
| FlashbackPlaybackController | Ready | `Sussudio/Services/Flashback/FlashbackPlaybackController.cs`, `Sussudio/Services/Flashback/FlashbackPlaybackController.ThreadCommands.cs`, and `Sussudio/Services/Flashback/FlashbackPlaybackController.PlaybackFrames.cs` now align to public playback state/commands/metrics, playback-thread command execution, and frame decode/submit pacing. Existing Flashback contract tests and this map document command, frame, audio-master, marker, and lifecycle ownership. | Treat the three-file split as intentional. Reopen only for a real collaborator with a fixture or lifetime boundary, not for line count. Preserve playback command names, queue telemetry, live-restore behavior, A/V drift policy, and segment-edge recovery. |
...
- `CaptureService.Flashback.cs` also owns Flashback export entry points,
...
- `FlashbackExporter.cs` owns the whole exporter: shared native export state,
  disposal/cancellation/lock handling, FFmpeg input/output context setup,
  stream-template/layout validation, public request routing/scheduling,
  single-file packet pumping, multi-segment template/preflight and packet
  rebasing, packet timestamp/buffer helpers, progress/pacing, output/path/range
  validation, temp/final output replacement, final result shaping, and FFmpeg
  error formatting. Keep this as one file while those helpers share the same
  active native contexts, temp output path, export lock, and packet lifetime
  state; split only if a named exporter collaborator with its own fixture and
  lifetime boundary replaces the private helper regions.
