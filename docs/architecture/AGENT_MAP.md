...
| Audio capture | `Sussudio/Services/Audio/WasapiAudioCapture.cs`, `WasapiAudioCapture.CaptureLoop.cs` | WASAPI state, endpoint binding/format negotiation/AudioClient startup, start/stop/dispose lifecycle, initialization-time metric resets, callback/glitch metric projection, capture thread/packet drain, WASAPI sample decode, f32le 48 kHz stereo conversion/resampling, pooled converted-packet buffers, converted-packet sink/playback/hot writer fan-out, and hot writer task-completion enforcement |
...
- `tests/Sussudio.Tests/D3D11PreviewRenderer.DiagnosticsContract.Tests.cs`
  ...
  `PerformanceTimelineEntry.cs` preview, Flashback playback, Flashback export,
