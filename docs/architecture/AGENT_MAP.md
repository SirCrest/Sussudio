...
| Source telemetry | `Sussudio/Services/Telemetry/NativeXuAtCommandProvider.cs`, `NativeXuAtCommandProvider.AtProtocol.cs`, `NativeXuAtCommandProvider.AudioCommands.cs`, `NativeXuAtCommandProvider.DeviceCommands.cs`, `NativeXuAtCommandProvider.DiagnosticSummary.cs`, `NativeXuAtCommandProvider.FullSnapshot.cs`, `NativeXuAtCommandProvider.PayloadDecoding.cs`, `NativeXuAtCommandProvider.RollingPoll.cs`, `NativeXuAtCommandProvider.TelemetryDetails.Build.cs`, `NativeXuAtCommandProvider.TelemetryDetails.AudioInput.cs`, `NativeXuAtCommandProvider.TelemetryDetails.Formatters.cs` | Native XU selected-interface validation and ReadAsync orchestration, AT-command transport/parsing, HDMI/Analog audio route and gain command sequences, generic public device-command surface, diagnostic summary formatting, reference full-snapshot reader, source payload decoding/scalar helpers, active rolling poll/snapshot assembly, source telemetry detail row assembly, flash-audio input interpretation, AT detail value formatting |
...
- `tests/Sussudio.Tests/D3D11PreviewRenderer.SourceOwnership.Tests.cs` is the
  renderer source-ownership marker shell. `D3D11PreviewRenderer.SourceOwnership.FrameFlow.Tests.cs`
