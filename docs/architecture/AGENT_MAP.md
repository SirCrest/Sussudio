...
| Recording | `Sussudio/Services/Recording/LibAvEncoder.cs`, `LibAvEncoder.Audio.cs`, `LibAvEncoder.CodecPolicy.cs`, `LibAvEncoder.AvSync.cs`, `LibAvEncoder.PacketWriting.cs`, `LibAvEncoder.VideoSetup.cs`, `LibAvEncoder.OutputLifecycle.cs`, `LibAvRecordingSink.cs`, `LibAvRecordingSink.Queues.cs`, `RecordingVerifier.cs`, `RecordingVerifier.Cadence.cs` | encoder runtime/lifecycle, audio/microphone stream setup and sample drains, codec/options policy, A/V sync diagnostics, video packet drain/write helpers, video codec/hardware setup, rotation/output cleanup, sink lifecycle/options/drain loop, recording sink queue surface, verifier/finalizer, verifier cadence analysis |
