...
| WASAPI interop | `Sussudio/Services/Audio/WasapiComInterop.cs`, `WasapiComInterop.CoreAudio.Contracts.cs`, `WasapiComInterop.AudioClient.Contracts.cs` | helper methods, format allocation/parsing, COM activation/release, endpoint volume, AudioClient3 initialization, shared audio format/PROPVARIANT structs, Core Audio device/property contracts, and AudioClient/capture/render/endpoint-volume contracts |
