...
- `Sussudio/Services/Preview/D3D11PreviewRenderer.Diagnostics.cs` owns recent
  ...
  ring writer, and slow-frame reason token classification.
  `D3D11PreviewRenderer.SlowFrameDxgiSlip.cs` owns the DXGI refresh-slip
  snapshot used by slow-frame diagnostics.
