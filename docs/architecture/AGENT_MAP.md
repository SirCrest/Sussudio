...
| Flashback | `FlashbackPlaybackController.cs`, `FlashbackPlaybackController.Markers.cs`, `FlashbackPlaybackController.PositionMapping.cs`, `FlashbackPlaybackController.Metrics.cs`, `FlashbackEncoderSink.cs`, `FlashbackEncoderSink.Options.cs`, `FlashbackEncoderSink.Queues.cs`, `FlashbackExporter.cs`, `FlashbackExporter.Infrastructure.cs` | playback core, marker owner, position/file-PTS mapping, metrics, buffer, encoder runtime, encoder options/packet helpers, encoder queue helpers, export modules, export infrastructure |
