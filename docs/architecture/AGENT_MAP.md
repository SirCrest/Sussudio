...
| AutomationDiagnosticsHub | Needs Boundary Pass | `Sussudio/Services/Automation/AutomationDiagnosticsHub.SnapshotProjection.cs` is still the largest production file in the generated baseline, while `Sussudio/Services/Automation/AutomationDiagnosticsHub.cs`, `Sussudio/Services/Automation/AutomationDiagnosticsHub.Snapshots.cs`, and `Sussudio/Services/Automation/AutomationDiagnosticsHub.Evaluation.cs` already own polling, stateful refresh/alerts, and diagnostic verdict logic. The current `tests/Sussudio.Tests/ArchitectureGuardrails.Tests.cs` and `tests/Sussudio.Tests/XUnit.AutomationContractsTests.cs` coverage protects wire fields and classifier wiring, but the projection file remains a large private flattener. | The next non-cosmetic slice should carve a named projection collaborator or projection-set owner only if it keeps the `AutomationSnapshot` wire initializer stable and preserves CLI/MCP/tool contracts. Until then, keep changes local to the four-file hub family and update source-shape tests with any moved projection surface. |
