...
| MJPEG preview pacing | `Sussudio/Services/Capture/MjpegPreviewJitterBuffer.cs`, `MjpegPreviewJitterBuffer.Metrics.cs` | decoded preview-frame queueing/deadline policy, jitter-buffer metric records and timing sample projection |
...
| Preview rendering | `D3D11PreviewRenderer.cs`, `D3D11PreviewRenderer.Rendering.cs`, `D3D11PreviewRenderer.Resources.cs`, `D3D11PreviewRenderer.Metrics.cs`, `D3D11PreviewRenderer.ScreenshotCapture.cs`, `D3D11PreviewRenderer.ShaderSources.cs` | renderer host, render loop/draw paths, D3D resources and swap-chain binding, present/latency metrics, screenshot capture, shader source, timing models |
