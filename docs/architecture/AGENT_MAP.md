...
| Capture leaf models | `Sussudio/Models/Capture/CaptureModels.cs`, `CaptureSettings.cs`, `CaptureSessionTransitionPolicy.cs` | device/options/session-state leaf types plus frame-ledger event DTOs kept together as the small capture model surface, capture settings contract, explicit transition legality policy, and mutable capture session state machine |
...
- `CaptureSessionTransitionPolicy.cs` owns pure transition legality,
