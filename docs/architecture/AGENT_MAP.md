...
| MJPEG preview pacing | `Sussudio/Services/Capture/MjpegPreviewJitterBuffer.cs`, `MjpegPreviewJitterBuffer.FrameIngress.cs`, `MjpegPreviewJitterBuffer.FramePacing.cs`, `MjpegPreviewJitterBuffer.Metrics.cs` | construction, suppression, disposal lifecycle, paced emit loop control flow, decoded preview-frame ingress, pooled payload ownership, queue ordering and reprime recovery, display-clock alignment, renderer submission, adaptive deadline/depth policy, jitter-buffer metric records, and timing sample projection |
| MJPEG decode pipeline | `Sussudio/Services/Gpu/ParallelMjpegDecodePipeline.cs`, `ParallelMjpegDecodePipeline.Reorder.cs`, `ParallelMjpegDecodePipeline.Lifecycle.cs`, `ParallelMjpegDecodePipeline.Metrics.cs`, `SoftwareMjpegDecoder.cs`, `NvdecMjpegDecoder.cs`, `CudaD3D11Interop.Initialization.cs`, `CudaD3D11Interop.Copy.cs` | pipeline construction/startup sequencing, bounded work-channel construction, compressed input admission/byte budget/depth accounting, CPU MJPEG worker decode-loop execution and decoder ownership, decoded-frame ordering, missing-sequence state, decoded-frame emission and preview notification, stop/dispose/shutdown joins/fatal callback signaling, decoder/work-item/reorder-frame resource cleanup, pipeline timing and packet-hash metrics, software MJPEG decoder initialization/lifetime, decode/copy hot path, NVDEC decoder state, standalone CUDA device/frame-pool initialization, shared CUDA device/frame-pool adoption, decode/context access, CPU download/copy helpers, disposal, and error text, CUDA-to-D3D11 bridge state, public texture handles, bridge setup/zero-copy registration, bridge disposal/resource unregister, CUDA native constants/P/Invoke declarations, and zero-copy/staging copy behavior |
...
- `UnifiedVideoCapture.Initialization.cs` owns source-reader/D3D/MJPEG
  initialization, CPU MJPEG decode pipeline construction, preview jitter buffer
  setup, and committed runtime state reset.
- `UnifiedVideoCapture.Lifecycle.cs` owns read-loop start/stop, preview-reinit
  disposal, CPU MJPEG pipeline stop/retention semantics, preview jitter buffer
  disposal, and capture/MJPEG fatal-error callbacks.
...
- `tools/Common/DiagnosticSessionModels.cs` owns diagnostic session run
  ...
  state, and immutable scenario phase result handoff consumed by completion.
