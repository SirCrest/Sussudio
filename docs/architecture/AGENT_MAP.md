...
| Capture snapshot models | `Sussudio/Models/Capture/CaptureDiagnosticsSnapshot.cs`, `CaptureDiagnosticsSnapshot.SourceTelemetry.cs`, `CaptureDiagnosticsSnapshot.CaptureCadence.cs`, `CaptureDiagnosticsSnapshot.Recording.cs`, `CaptureDiagnosticsSnapshot.Flashback.cs`, `CaptureDiagnosticsSnapshot.Mjpeg.cs`, `CaptureHealthSnapshot.cs` | diagnostics core/format/HDR compatibility fields, source telemetry fields, capture cadence fields, recording/audio queue fields, Flashback queue fields, MJPEG and visual-cadence diagnostics fields, consolidated health source/queue/AV-sync and Flashback backend/playback/export health fields |
