...
- `tests/Sussudio.Tests/RecordingQueue.LibAvSink.Tests.cs` owns LibAv recording
  sink output validation, try-enqueue, video-session setup, queue-cleanup,
  drain-loop, encoding-loop, startup sequencing, stop-lifecycle, and
  lifetime-helper ownership assertions.
