...
| Audio capture | `Sussudio/Services/Audio/WasapiAudioCapture.cs`, `WasapiAudioCapture.CaptureLoop.cs`, `WasapiAudioCapture.Conversion.cs`, `WasapiAudioCapture.Diagnostics.cs` | WASAPI state, endpoint binding/format negotiation/AudioClient startup, start/stop/dispose lifecycle, capture thread/packet drain plus converted-packet sink/playback/hot writer fan-out, f32le 48 kHz stereo conversion/resampling helpers, and callback/glitch metric projection |
...
- `DeviceService.FormatProbe.cs` owns inline/background Media Foundation format
  probing, persisted format-cache DTOs and load/save/delete helpers, and
  pixel-format/frame-rate normalization.
...
- `CaptureService.FlashbackSettings.cs` owns buffer-cycle transition
...
- `LibAvEncoder.HardwareSubmission.cs` owns D3D11 and CUDA hardware-frame
  ...
  checks, hardware-frame PTS/keyframe assignment, shared HDR side-data helper
  calls, EAGAIN packet drains, and hardware-frame unref cleanup.
...
- `Sussudio/Services/Recording/Verification/RecordingVerifier.cs` owns strict verification orchestration, early
  failure results, primary mismatch parsing, HDR parity, mismatch taxonomy, and the public verifier surface.
...
- `tests/Sussudio.Tests/McpToolSurface.DiagnosticSession.Flashback.HealthPolicy.Tests.cs`
  owns diagnostic-session Flashback warmup health-policy, warning-policy, and
