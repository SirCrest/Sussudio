...
- `Sussudio/Services/Preview/D3D11PreviewRenderer.ScreenshotCapture.cs` owns
  ...
  present, error result construction, capture-result logging, and off-thread PNG
  completion/encode-gate state. `D3D11PreviewRenderer.ScreenshotStaging.cs`
  owns staging-texture reuse, resize, and teardown.
