...
- `CaptureService.Failures.cs` owns fatal capture/recording/Flashback backend
  failure callbacks, fatal capture cleanup launch and generation-stale guards,
  last-failure telemetry state fields, lock, mutation helpers, clear helpers,
  and snapshot reads. It routes cleaning-up/faulted transitions through
