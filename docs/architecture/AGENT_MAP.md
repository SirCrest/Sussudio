...
| Audio playback | `Sussudio/Services/Audio/WasapiAudioPlayback.cs`, `WasapiAudioPlayback.Queue.cs`, `WasapiAudioPlayback.RenderThread.cs` | playback state, render endpoint binding/format validation/AudioClient startup, start/stop/pause/resume/flush/dispose lifecycle, chunk queue and buffered-duration accounting, render-thread callback/prebuffer/buffer-fill execution, render-side PTS advancement, volume ramps, and output-level telemetry |
