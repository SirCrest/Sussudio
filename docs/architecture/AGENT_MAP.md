...
- `FlashbackEncoderSink.ForceRotateRequests.cs` owns export force-rotate state/status projections, idle waits, request admission/publication, timeout/cancellation result classification, committed-pending grace handling, pending-request cancellation, empty completion on stop/dispose/failure, force-rotate drain abort policy, and the `ForceRotateRequest` state machine.
