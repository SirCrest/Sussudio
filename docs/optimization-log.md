...
- Diagnostic improvement: playback decode snapshots now include the playback position and UTC timestamp for the decode sample that owns the current max decode phase bundle. This should tell the next long run whether the reported `send_packet` max actually coincides with the 1% low dip or is a cumulative earlier max while visual cadence remains clean.
- Follow-up evidence: the next 180-second `flashback-playback` run in `temp\diag-flashback-playback-20260504-0320-decodepos-hidden` kept visual cadence at 119.99fps with high-motion confidence and zero playback drops/submit failures, while the sampled playback 1% low briefly dipped to 89.85fps around offset 50.377s. The new max-decode telemetry pinned the cumulative worst decode sample to playback position 1571ms, phase `send`, max 16.7359ms. This supports treating the playback 1% low dip as a narrow decode/feed stall or sampled-metric artifact rather than a 120fps-to-60fps playback mode issue.
- Change: high-rate adjacent `Seek` and `UpdateScrub` commands now use per-slot latest-intent patterns, so repeated timeline motion can fold stale positions without crossing later `Play`, `Pause`, `GoLive`, `EndScrub`, or opposite-mode control barriers.
- Change: `BeginScrub` and plain `EndScrub()` now snapshot the latest user-intended scrub target at the command-order boundary, so releasing a scrub lands on the dragged position instead of the last decoded/keyframe-snapped `PlaybackPosition`.
