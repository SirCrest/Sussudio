...
public partial class MainViewModel : ObservableObject, IDisposable, IAsyncDisposable, IAutomationViewModel
{
    ...
    public MainViewModel()
    {
        ...
        // resume. SystemResuming fires on a thread-pool thread; the handler
        // dispatches the actual reinit work to the UI thread via EnqueueUiOperation.
        PowerManager.SystemResuming += OnSystemResuming;
    ...
    // SystemResuming fires on the system thread pool — must not touch UI properties
    // directly. All UI-state reads happen inside the EnqueueUiOperation lambda,
    // which executes on the DispatcherQueue thread. ReinitializeDeviceAsync's
    // IsRecording guard (fix #1) keeps this safe to call regardless of state.
    private void OnSystemResuming(object? sender, object? args)
    {
