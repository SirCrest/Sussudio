...
internal sealed class WasapiAudioCapture : IAsyncDisposable
{
    ...
    public Task InitializeAsync(string audioDeviceId, CancellationToken ct)
    {
        ...
        {
            Logger.Log($"Cleaning up and rethrowing from WasapiAudioCapture.InitializeAsync: {ex.Message}");
    ...
    private void StartCore(TaskCompletionSource<bool>? recordingStartupReady)
    {
        ...
        catch (Exception ex)
        {
            ...
            Logger.Log($"Cleaning up and rethrowing from WasapiAudioCapture.StartCapture: {ex.Message}");
    ...
    private void OnCaptureFailed(Exception ex)
    {
        ...
        {
            Logger.Log($"Suppressed exception in WasapiAudioCapture event fan-out: {fanOutEx.Message}");
        }
    }
