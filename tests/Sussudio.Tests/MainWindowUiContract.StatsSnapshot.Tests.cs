...
static partial class Program
{
    private static Task StatsSnapshotConstruction_LivesInFocusedBuilder()
    {
        ...
        AssertContains(statsSnapshotProviderText, "internal sealed partial class StatsSnapshotProvider");
        AssertContains(statsSnapshotProviderRenderMetricsText, "internal sealed partial class StatsSnapshotProvider");
        AssertContains(statsSnapshotProviderRenderMetricsText, "private const int RecentSampleCount = 180;");
        ...
        AssertContains(statsSnapshotProviderRenderMetricsText, "var presentCadence = renderer?.GetPresentCadenceMetrics(previewMinPresentationIntervalMs);");
        AssertContains(statsSnapshotProviderRenderMetricsText, "PreviewRecentPresentIntervalsMs: renderer?.GetRecentPresentIntervalsMs(RecentSampleCount) ?? Array.Empty<double>()");
        AssertDoesNotContain(statsSnapshotProviderText, "var presentCadence = renderer?.GetPresentCadenceMetrics(previewMinPresentationIntervalMs);");
        AssertDoesNotContain(statsSnapshotProviderText, "PreviewRecentPresentIntervalsMs: renderer?.GetRecentPresentIntervalsMs(RecentSampleCount) ?? Array.Empty<double>()");
