...
// MainWindow Flashback automation and presentation contracts live with their xUnit wrappers.
    ...
    internal static Task MainViewModelAutomation_UsesAsyncFlashbackAndProbeSurface()
    {
        ...
        AssertContains(dispatcherText, "FlashbackExportFailure.GetKind(exportResult)");
    ...
    private static void AssertDiagnosticsRefreshFlashbackExportOwnership(string dispatcherText)
    {
        ...
        AssertContains(captureServiceText, "return FailFlashbackExport(outputPath, \"Flashback export cancelled.\", FlashbackExportFailure.Cancelled, inPoint, outPoint);");
        ...
        AssertContains(captureServiceText, "var failure = FlashbackExportFailure.Create(");
        ...
        AssertContains(captureServiceText, "FlashbackExportFailure.GetKind(result)");
        ...
        AssertContains(captureServiceText, "return FailFlashbackExport(outputPath, \"Flashback buffer not active\", FlashbackExportFailure.BufferInactive, inPoint, outPoint);");
        ...
        AssertContains(captureServiceText, "if (!double.IsFinite(seconds) || seconds <= 0 || seconds > TimeSpan.MaxValue.TotalSeconds)");
        AssertContains(captureServiceText, "return FailFlashbackExport(outputPath, \"Flashback export cancelled.\", FlashbackExportFailure.Cancelled);");
        AssertContains(captureServiceText, "return FailFlashbackExport(outputPath, \"Flashback export duration must be finite, greater than zero, and within TimeSpan range.\", FlashbackExportFailure.InvalidRequest);");
        AssertRegex(
            dispatcherText,
        ...
        AssertContains(captureServiceText, "FlashbackExportFailure.IsCancelled(result)");
