static partial class Program
{
    private static void AssertMcpPerformanceTimelineSourceOwnership(McpPerformanceTimelineSources sources)
    {
        ...
        AssertDoesNotContain(sources.RowsSource, "MjpegPreviewJitter");
        AssertDoesNotContain(sources.RowsSource, "FlashbackPlaybackState =");
        AssertDoesNotContain(sources.RowsSource, "FlashbackExportActive =");
        AssertDoesNotContain(sources.RowsSource, "GcPause =");
        AssertContains(sources.RowsPreviewSource, "private static void PopulatePreviewTimelineRow(JsonElement item, TimelineRow row)");
        AssertContains(sources.RowsFlashbackPlaybackSource, "private static void PopulateFlashbackPlaybackTimelineRow(JsonElement item, TimelineRow row)");
        AssertContains(sources.RowsFlashbackExportSource, "private static void PopulateFlashbackExportTimelineRow(JsonElement item, TimelineRow row)");
        AssertContains(sources.RowsSystemSource, "private static void PopulateSystemTimelineRow(JsonElement item, TimelineRow row)");
        AssertContains(sources.RowsPreviewSource, "MjpegPreviewJitterLatencyP95Ms");
        AssertContains(sources.RowsFlashbackPlaybackSource, "FlashbackPlaybackMaxCommandQueueLatencyCommand");
        AssertContains(sources.RowsFlashbackExportSource, "FlashbackExportThroughputBytesPerSec");
        AssertContains(sources.RowsSystemSource, "ThreadPoolIoAvailable");
        AssertDoesNotContain(sources.RowsPreviewSource, "FlashbackPlayback");
        AssertDoesNotContain(sources.RowsFlashbackPlaybackSource, "FlashbackExportActive");
        AssertDoesNotContain(sources.RowsFlashbackExportSource, "GcPause");
        AssertDoesNotContain(sources.RowsSystemSource, "PreviewCadence");
        ...
        AssertContains(sources.RowsPreviewModelSource, "public string PreviewPacingSlowStageEvidence { get; set; } = string.Empty;");
        AssertContains(sources.RowsFlashbackPlaybackModelSource, "public string FlashbackPlaybackLastCommandFailure { get; set; } = string.Empty;");
        AssertContains(sources.RowsFlashbackExportModelSource, "public double FlashbackExportThroughputBytesPerSec { get; set; }");
        AssertContains(sources.RowsSystemModelSource, "public int IoThreads { get; set; }");
