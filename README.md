...
- **Automated** A named-pipe IPC server plus the `ssctl` CLI and an
  MCP bridge expose full app state, so scripts (and Claude Code) can drive
  preview, recording, and verification end-to-end. And control through Stream Deck.
...
- Live D3D11 preview (GPU path) with optiona software decode
