...
public static partial class AutomationCommandCatalog
{
    ...
    private static void RegisterCaptureEntries(Dictionary<AutomationCommandKind, AutomationCommandMetadata> entries)
    {
    ...
    private static void RegisterUiEntries(Dictionary<AutomationCommandKind, AutomationCommandMetadata> entries)
    {
        ...
        Set(entries, AutomationCommandKind.SetShowAllCaptureOptions, "{ enabled: bool }", ready: false, timeoutMs: DefaultTimeout, pathPolicy: AutomationCommandPathPolicy.None, "set show-all on|off", "Show-all capture options are always enabled; this compatibility command is accepted as a no-op.", Required("enabled", AutomationPayloadFieldType.Boolean));
